...
namespace ChessV1
{
	...
	public class Calculation2
	{
		...
		public Calculation2(Dictionary<int[], PieceType> position, Turn turnColor, int depth)
		{
			Position = position;
			TurnColor = turnColor;
			Depth = depth;
			...
			{
				var allLegalMoves = GetAllLegalMoves(Position, turnColor);
			}
			...
			CalculateBestMove(new List<List<Dictionary<int[], PieceType>>>() { new List<Dictionary<int[], PieceType>>() { Position } }, TurnColor, 0);
		}

		private void CalculateBestMove(List<List<Dictionary<int[], PieceType>>> currentLines, Turn turnColor, double currentDepth)
		{
			if (currentDepth >= Depth || currentLines.Count == 0) return;	// Calculation complete

			// First, get all legal moves for current Color and do them
			foreach (var position in currentLines)
			{

			}
			// Then, for all resulting lines, 
		}
	...
	public class Calculation
	{
		...
		internal Calculation(Dictionary<int[], PieceType> position, Turn turnColor, int depth)
		{
	...
	// This move history does not account for en passant
	public class MoveHistory
	{
		...
		{
			if (maxmove == 0) return DefaultInitialPosition;
		...
		public void AddNormalMove(KeyValuePair<int[], int[]> Move) => AddMove(Move, 'n');
		public void AddEnPassantMove(KeyValuePair<int[], int[]> Move) => AddMove(Move, 'e');
		public void AddCastlesKingMove(KeyValuePair<int[], int[]> Move) => AddMove(Move, 'c');
