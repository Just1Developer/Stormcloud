...
using System.Linq;
using System.Text;
using ChessV1.Stormcloud.Connect4.Weightfinder.JSON_Export;

namespace ChessV1.Stormcloud.Connect4.Weightfinder.PSO_JSON_WRITER
{
	internal class PSOExportParser
	{
		internal static void ConvertToJSONs(int ParticleRunID, int ParticleDimensions = 11)
			=> ConvertToJSONs($"{ParticleSwarmOptimization.DATA_PATH}particle_{ParticleRunID}", ParticleDimensions);
		internal static void ConvertToJSONs(string ParticlePath, int ParticleDimensions = 11)
		{
			var historyItem = LoadHistory(ParticlePath, ParticleDimensions);
			...
			string newFolder = ParticlePath.Replace("Buffer2", "Buffer3");
			if (!Directory.Exists(newFolder)) Directory.CreateDirectory(newFolder);

			foreach (var _history in history)
			{
				_history.ExportJSON(newFolder);
			}

			ExportJSONGlobalBest(newFolder, globalWeights, globalScore);
		}
		...
		internal static void ConvertToJSONSingleFile(int ParticleRunID, int ParticleDimensions = 11)
			=> ConvertToJSONSingleFile($"{ParticleSwarmOptimization.DATA_PATH}particle_{ParticleRunID}", ParticleDimensions);
		internal static void ConvertToJSONSingleFile(string ParticlePath, int ParticleDimensions = 11)
		{
			var historyItem = LoadHistory(ParticlePath, ParticleDimensions);
			...
			string file = newFolder + "history.json";
		...
		internal static void ConvertToJSONSingleFileInvertedOrder(int ParticleRunID, int ParticleDimensions = 11)
			=> ConvertToJSONSingleFileInvertedOrder($"{ParticleSwarmOptimization.DATA_PATH}particle_{ParticleRunID}", ParticleDimensions);
		internal static void ConvertToJSONSingleFileInvertedOrder(string ParticlePath, int ParticleDimensions = 11)
		{
			...
			string newFolder = ParticlePath.Replace("Buffer2", "Buffer4");
			if (!Directory.Exists(newFolder)) Directory.CreateDirectory(newFolder);
			...
			{
				h++;
				write.Write(_history.ToString(h < history.Length));
			}
			...
			ExportJSONGlobalBest2(newFolder, globalWeights, globalScore);
		}
		...
		static void ExportJSONGlobalBest(string folder, double[] GlobalWeights, double GlobalScore)
		{
			string file = folder + $"/global-best.json";

			StringBuilder ScoreBuilder = new StringBuilder();
			StringBuilder[] WeightBuilders = new StringBuilder[11];
			...
			AppendAll("		\"");
			for (int i = 0; i < 11; i++)
				WeightBuilders[i].Append($"Weight_{i+1}");
			...
			for (int i = 0; i < 11; i++)
			{
				WeightBuilders[i].Append($"{GlobalWeights[i]}".Replace(",", "."));
			}
			ScoreBuilder.Append($"{GlobalScore}".Replace(",", "."));
		...
		static void ExportJSONGlobalBest2(string folder, double[] GlobalWeights, double GlobalScore)
		{
			string file = folder + $"/global-best.json";
			...
			AppendAll("	\"");
			for (int i = 0; i < 11; i++)
				WeightBuilders[i].Append($"weight{i+1}");
			ScoreBuilder.Append("score");
			AppendAll("\": ");
